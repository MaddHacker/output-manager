 ...
 */
var __doLog = function (msg, level) {
 ...
 */
function getLogOutIfLevelAllows() {
    return function (msg, level) {
        if (__logLevel.value <= level.value) {
            __logger(__fmtString(level, msg));
        }
    };
}
 ...
 */
var __fmtString = function (level, msg) {
    return '%{0} [%{1}] %{2}'.fmt(date(), level.name, msg);
};
